...
class IMMSmoother:
    ...
    def mode_interaction_pre( 
        ...
        for i in range(M): 
            ...
            x_premixed_e, P_premixed_e = filters[i].to_external(x_premixed[i], P_premixed[i])

            # Covariance Intersection
            # Use a fixed omega=0.5 (this could be made adaptive if desired)
            from scipy.optimize import minimize_scalar

            def ci_objective(omega):
                # Clamp omega for safety
                omega = np.clip(omega, 0.0, 1.0)
                P_ci_inv = omega * np.linalg.inv(P_rts_e) + (1 - omega) * np.linalg.inv(P_premixed_e)
                try:
                    P_ci = np.linalg.inv(P_ci_inv)
                except np.linalg.LinAlgError:
                    return np.inf
                # We use logdet for numerical stability (minimize trace or det, logdet is popular)
                return np.log(np.linalg.det(P_ci) + 1e-12)  # add epsilon to avoid log(0)

            # Find omega that minimizes the logdet of the intersection covariance
            res = minimize_scalar(ci_objective, bounds=(0.0, 1.0), method='bounded')
            omega_opt = np.clip(res.x, 0.0, 1.0)
            print(omega_opt)

            P_ci_inv = omega_opt * np.linalg.inv(P_rts_e) + (1 - omega_opt) * np.linalg.inv(P_premixed_e)
            P_ci = np.linalg.inv(P_ci_inv)
            x_ci = (
                omega_opt * (np.linalg.inv(P_rts_e) @ x_rts_e) +
                (1 - omega_opt) * (np.linalg.inv(P_premixed_e) @ x_premixed_e)
            )
            x_ci = P_ci @ x_ci

            # Store the CI "backward" information as matrix/vector
            back_info_matrices.append(P_ci_inv)
            back_info_vectors.append(P_ci_inv @ x_ci)
        invertible = True
        for i in range(M):
