...
class IMMSmootherRTS:
        ...
        for k in range(K-2, -1, -1):
            ...
            for j in range(M):
                for i in range(M):
                    ...
                    xi_c, Pi_c = imm.models[i].to_external(
            ...
            for j in range(M):
                val = 0.0
                x_pred_c, P_pred_c = imm.models[j].to_external(
                    hist[k+1]["models"][j]["x_pred"],
                    hist[k+1]["models"][j]["P_pred"],
                )
                for i in range(M):
                    xi_c, _ = imm.models[i].to_external(
                        x_mode[k+1][i], P_mode[k+1][i]
                    )
                    y = xi_c - x_pred_c
                    # print(f"y: {y}")
    ...
    def _moment_match(self, imm, x_modes, P_modes, mu):
        ...
        for j, model in enumerate(imm.models):
            xj, Pj = model.to_external(x_modes[j], P_modes[j])
            x += mu[j] * xj
